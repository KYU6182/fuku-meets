...
const cafeRanking = [
  { rank: 1, image: "/images/cafe-1.jpg", title: "manu coffee", area: "大名・赤坂", description: "朝から夜までふらっと寄れる、福岡らしい距離感の定番カフェ。", votes: "1,245票" },
  { rank: 2, image: "/images/cafe-2.jpg", title: "café mitu", area: "薬院", description: "落ち着いた空気と丁寧な一杯で、週末の予定に入れたい場所。", votes: "987票" },
  { rank: 3, image: "/images/cafe-3.jpg", title: "Sabrina Coffee", area: "薬院", description: "友だちにも一人時間にも使いやすい、街に馴染むコーヒースタンド。", votes: "742票" },
];

const dailyRanking = [
  { title: "好きなスーパー TOP3", items: [{ rank: 1, name: "ボンラパス", votes: "1,156票" }, { rank: 2, name: "サニー", votes: "978票" }, { rank: 3, name: "にしてつストア", votes: "845票" }] },
  { title: "モデル TOP3", items: [{ rank: 1, name: "YUI", votes: "1,024票" }, { rank: 2, name: "RENA", votes: "892票" }, { rank: 3, name: "KEITA", votes: "732票" }] },
];
...
const weekendSelects = [
  { image: "/images/cafe-2.jpg", label: "CAFE", title: "薬院の隠れ家カフェ3選", description: "ゆったり過ごせる、隠れ家カフェまとめ。" },
  { image: "/images/ramen.jpg", label: "RAMEN", title: "大名の深夜ラーメン", description: "夜中に食べたくなる、あの一杯。" },
  { image: "/images/club.jpg", label: "CLUB", title: "週末クラブアドレス", description: "音楽と人に出会う、週末のおすすめ。" },
];
...
export default function HomePageClient() {
  const [cms, setCms] = useState<HomeCmsData>(defaultHomeCmsData);

  useEffect(() => {
    const saved = window.localStorage.getItem(storageKeys.adminHomePublished);
    setCms(mergeHomeCmsData(saved ? JSON.parse(saved) : null));
  }, []);

  function renderSection(sectionId: HomeSectionId) {
    switch (sectionId) {
      case "hero":
        return <HeroSection hero={cms.hero} />;
      case "fukuIcons":
        return <FukuIconsSection iconsData={iconsData} />;
      case "ranking":
        return <RankingSection rankingCategories={rankingCategories} cafeRanking={cafeRanking} dailyRanking={dailyRanking} />;
      case "weekendGuide":
        return <WeekendGuideSection weekendCategories={weekendCategories} weekendSelects={weekendSelects} />;
      case "newInFukuoka":
        return <NewInFukuokaSection newInFukuokaItems={newInFukuokaItems} />;
      case "magazine":
        return <PaperSection />;
      case "pickupContents":
        return <PickUpContentsSection pickupContents={pickupContents} />;
      default:
        return null;
    }
  }
  ...
  return (
    <div className="mx-auto min-h-screen max-w-[430px] bg-white shadow-phone">
      <Header />
      <main>
        {cms.sections.filter((section) => section.isVisible).map((section) => (
