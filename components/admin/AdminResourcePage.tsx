  ...
  const base = {
    news: {
      ...
      fields: [
        ...
        { label: "記事タイプ", type: "select" as const, value: (news as any)?.type ?? "news", options: ["news", "guide", "shop_guide", "live_guide", "visitor_guide"] },
...
function buildPayload(kind: EditableKind, id: string | undefined, values: Record<string, string>, status: string, entries: RankingEntryForm[]) {
  if (kind === "news") {
    return {
      ...
      displayOrder: Number(values["表示順"] || 0),
      status,
    };
  }
  if (kind === "icons") {
